...
def build_FSM(model, X, Y, K=3, T=30, F=0.1, details=False):
    r"""
        Arguments:
...
def run_FSM(X, Y, abstract_alphabet, initial_vector, transition_matrices, final_vector):
    r"""
        Arguments:
            X (numpy.array): time series data (sample_amount, time_step, feature_dim)
            Y (numpy.array): label (sample_amount, )
            abstract_alphabet (numpy.array): abstract tokens set, from build_FSM()
            initial_vector (numpy.array):  initial state of WFA, from build_FSM()
            transition_matrices (numpy.array): record transitions of WFA under different tokens, from build_FSM()
            final_vector (numpy.array): give prediction from output, from build_FSM()
    """
    output_size = int(np.max(Y) + 1)
...
if __name__ == '__main__':
    ...
    abst_alphabet, initial_vec, trans_matrices, final_vec = build_FSM(model, X, Y, K=3, T=30, F=0.1, details=True)
    wfa_output = run_FSM(X, Y, abst_alphabet, initial_vec, trans_matrices, final_vec)
